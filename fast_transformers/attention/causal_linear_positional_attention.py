...
class CausalLinearPositionalAttention(Module):
    ...
    def forward(self, queries, keys, values, queries_pos, keys_pos, attn_mask,
        ...
        Pq, Pk = self._make_sizes_compatible(Pq, Pk)
        ...
        Z = 1 / (
            torch.einsum("nlhi,nlhi->nlh", Q, K.cumsum(1)) +
            torch.einsum("nlhi,nlhi->nlh", Pq, Pk.cumsum(1)) +
            self.eps)

        # Compute the unnormalized result
        V = causal_linear(Q, K, values) + causal_linear(Pq, Pk, values)
