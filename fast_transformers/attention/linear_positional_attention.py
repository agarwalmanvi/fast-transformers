...
"""Implement unmasked linear attention with positional embeddings added in the
key space."""
...
class LinearPositionalAttention(Module):
    ...
    def forward(self, queries, keys, values, queries_pos, keys_pos, attn_mask,
        ...
        PkV = torch.einsum("nshd,nshm->nhmd", Pk, values)

        # Compute the normalizer
        Z = 1 / (
            torch.einsum("nlhd,nhd->nlh", Q, K.sum(dim=1)) +
            torch.einsum("nlhd,nhd->nlh", Pq, Pk.sum(dim=1)) +
            self.eps)

        # Finally compute and return the new values
        V = (torch.einsum("nlhd,nhmd,nlh->nlhm", Q, KV, Z) +
             torch.einsum("nlhd,nhmd,nlh->nlhm", Pq, PkV, Z))
